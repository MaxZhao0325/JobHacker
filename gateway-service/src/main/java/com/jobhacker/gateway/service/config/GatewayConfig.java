package com.jobhacker.gateway.service.config;

import com.jobhacker.config.GatewayServiceConfigData;
...
//                .getRequest().getHeaders().getFirst(HEADER_FOR_KEY_RESOLVER)));
        return exchange -> Mono.justOrEmpty(exchange.getRequest().getHeaders().getFirst(HEADER_FOR_KEY_RESOLVER))
                .switchIfEmpty(Mono.just("default-key"));  // Fallback key if header is missing
    }
